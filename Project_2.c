...
void registration(my_variable *ptr_information, int *index1)
{
    int a = *index1;
    my_variable *ptr2;
    ptr2 = (my_variable *)malloc(1* sizeof(my_variable));
    printf("Name:");
    ...
    printf("<<%d>>",a+1); // it for printing the number of inputs
}
...
int main()
{
    ...
    scanf("%d", &max_number);
    my_variable *ptr;
    ptr = (my_variable*)malloc(max_number*sizeof(my_variable));
            if(ptr == NULL)
            {
              printf("NO Memory");
                return 1;
    ...
    while (instruction != 5)
    {
        if (instruction == 1)
        {
            registration(ptr, &index); //index goes as index of array
            index++;
            size++;
        }
        if (instruction == 2)
        {
            show_all(ptr, index); // index goes to function as size in for loop
        }
        if (instruction == 3)
        {
            delete(ptr, &index, size);
        }
        if (instruction == 4)
        {
            find_by_birth(ptr, index); // index goes to function as size in for loop
        }
        printf("\n*****Menu*****\n");
        printf("<1.Registration><2.ShowAll><3.Delete><4.FindByBirth><5.Exit>\n");
        printf("Enter_the_menu_number:");
